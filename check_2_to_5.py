...
def check_5_scam_database(
    ...
    strong_patterns = {
        "application fee": (
            r"\b(?:application|registration|"
            r"processing|joining)\s+fee\b"
        ),

        "training fee": (
            r"\btraining\s+(?:fee|payment|charge)\b"
        ),

        "payment for equipment": (
            r"\b(?:pay|payment|fee|deposit).{0,40}"
            r"\b(?:equipment|laptop|computer|starter kit)\b"
        ),
        ...
        "crypto payment": (
            r"\b(?:pay|payment|deposit).{0,30}"
            r"\b(?:crypto|bitcoin|cryptocurrency)\b"
        ),

        "zelle payment": (
            r"\b(?:pay|payment|send|transfer).{0,30}"
            r"\bzelle\b"
        ),
        ...
        "pay to get job": (
            r"\bpay\b.{0,40}\b(?:get|secure|receive)"
            r".{0,20}\bjob\b"
        ),

        "check deposit scheme": (
            r"\b(?:deposit|cash|mobile deposit)"
            r".{0,30}\bcheck\b"
        ),
    }

    medium_patterns = {
        "telegram contact": r"\btelegram\b",

        "whatsapp contact": r"\bwhatsapp\b",

        "starter kit": r"\bstarter\s+kit\b",
    ...
    for label, pattern in medium_patterns.items():
        if re.search(
            ...
            medium_matches.append(label)

    # Strong payment/scam indicators.
    if strong_matches:
        return {
            "passed": False,
            "points": 0,
            ...
            "reason": (
                "Strong scam/payment-pressure indicators "
                "were detected: "
                + ", ".join(
                    strong_matches
                )
    ...
    # Only medium-risk contact methods.
    if medium_matches:
        return {
            "passed": None,
            "points": 10,
            "severity": "medium",
            "matched_signatures": medium_matches,
            "reason": (
                "The offer contains communication or "
                "starter-kit patterns that deserve "
                "additional verification, but these "
                "patterns alone do not prove fraud."
            ),
        }
    ...
    return {
        ...
        "matched_signatures": [],
        "reason": (
            "No major payment-pressure or common scam "
